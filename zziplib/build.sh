...
FWKDIR="$PWD"
CONFIGOPTS="--disable-static --disable-mmap"
FWKS=(libzziplib)


# unpack source
if [ -d "$SRCDIR" ]; then rm -r "$SRCDIR"; fi
tar xjf zziplib-0.13.59.tar.bz2
mv "zziplib-0.13.59" "$SRCDIR"
...
for arch in ppc i386 x86_64; do
  ...
  for lib in "${FWKS[@]}"; do
    lname=${lib#lib}
    dyname=${lib/zziplib/zzip}
    install_name_tool -id "@executable_path/../Frameworks/$lname.framework/Versions/A/$lname" "$LIBDIR/$dyname.dylib"
    
    # fix links to sibling libraries
    for elib in "${FWKS[@]}"; do
      ename=${elib#lib}
      edyname=${lib/zziplib/zzip}
      install_name_tool -change "$LIBDIR/$edyname.dylib" "@executable_path/../Frameworks/$ename.framework/Versions/A/$ename" "$LIBDIR/$dyname.dylib"
    done
  done
done
...
for lib in "${FWKS[@]}"; do
  ...
  dyname=${lib/zziplib/zzip}
  lipo \
    "$INSTALLDIR/ppc/lib/$dyname.dylib" \
    "$INSTALLDIR/i386/lib/$dyname.dylib" \
    "$INSTALLDIR/x86_64/lib/$dyname.dylib" \
  ...
  for hfile in `find zzip -type f`; do
